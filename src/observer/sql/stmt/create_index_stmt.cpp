...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  for (int i = 0; i < static_cast<int>(create_index.attribute_names.size()); i++) {
  ...
  stmt = new CreateIndexStmt(table, create_index.unique, field_metas, create_index.index_name);
  return RC::SUCCESS;
}
