...
RC ExplainStmt::create(Db *db, const ExplainSqlNode &explain, Stmt *&stmt, SqlResult *&sql_result)
{
  Stmt *child_stmt = nullptr;
  RC    rc         = Stmt::create_stmt(db, *explain.sql_node, child_stmt, sql_result);
  if (rc != RC::SUCCESS) {
