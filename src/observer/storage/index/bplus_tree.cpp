    ...
    int leaf_max_size /* = -1 */)
{
  ...
  for (int i = 0; i < static_cast<int>(field_metas.size()); i++) {
    ...
    file_header->attr_length[i] = field_metas[i]->len();
  }
    ...
    int attr_length, int internal_max_size /* = -1 */, int leaf_max_size /* = -1 */)
{
  ...
  key_comparator_.init(file_header->unique, file_header->attr_type, file_header->attr_length, 1, 0);
  key_printer_.init(file_header->attr_type, file_header->attr_length, 1);
